...
(function (): void {
    const CARET =
        '<svg class="dropdown-caret" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" ' +
    ...
    function enhance(select: HTMLSelectElement): void {
        ...
        const items: HTMLLIElement[] = Array.from(select.options).map((option, index) => {
    ...
    document
        .querySelectorAll<HTMLSelectElement>('select.form-select, select.filter-select')
        .forEach(enhance);
})();
