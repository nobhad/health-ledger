...
type MultiselectType = 'condition' | 'trait' | 'gene';

function initMultiselect(type: MultiselectType, inputId: string, dropdownId: string, tagsId: string): void {
    ...
    const tags = document.getElementById(tagsId) as HTMLElement | null;
...
function updateMultiselectDropdown(type: MultiselectType, filter: string = ''): void {
    ...
    const state = (window.multiselectState && window.multiselectState[type]) ? window.multiselectState[type] : null;
    const dropdown = document.getElementById(type + 'Dropdown') as HTMLElement | null;
...
function toggleMultiselectOption(type: MultiselectType, value: string): void {
    ...
    updateMultiselectTags(type);
    const input = document.getElementById(type + 'Input') as HTMLInputElement | null;
    if (input) {
        updateMultiselectDropdown(type, input.value);
    } else {
...
function updateMultiselectTags(type: MultiselectType): void {
    // Always use window.multiselectState directly
    if (!window.multiselectState || !window.multiselectState[type]) {
        return;
    }
    ...
    const state = window.multiselectState[type];
    const tags = document.getElementById(type + 'Tags') as HTMLElement | null;
    ...
    state.selected.forEach((value: string) => {
        const option = (state.options && Array.isArray(state.options)) ? state.options.find((opt: MultiselectOption) => opt.value === value) : null;
        if (!option) {
            return;
        }
...
function removeMultiselectOption(type: MultiselectType, value: string): void {
    // Always use window.multiselectState directly
    if (!window.multiselectState || !window.multiselectState[type]) {
        return;
    }
