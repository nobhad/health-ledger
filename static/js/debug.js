...
// Info logger
window.infoLog = function (message, ...args) {
    if (!window.GENETIC_PROFILE_DEBUG.enabled) {
        return;
    }
...
// Performance logger
window.perfLog = function (operation, startTime) {
    if (!window.GENETIC_PROFILE_DEBUG.enabled) {
        return;
    }
...
// API call logger
window.apiLog = function (method, url, data, response) {
    if (!window.GENETIC_PROFILE_DEBUG.enabled) {
        return;
    }
    if (getCurrentLogLevel() > LOG_LEVELS.DEBUG) {
...
// State logger
window.stateLog = function (stateName, oldValue, newValue) {
    if (!window.GENETIC_PROFILE_DEBUG.enabled) {
        return;
    }
    if (getCurrentLogLevel() > LOG_LEVELS.DEBUG) {
...
// Element logger
window.elementLog = function (elementName, action, data) {
    if (!window.GENETIC_PROFILE_DEBUG.enabled) {
        return;
    }
    if (getCurrentLogLevel() > LOG_LEVELS.DEBUG) {
...
window.initDebugPanel = function () {
    ...
    if (levelSelect) {
        levelSelect.addEventListener('change', (e) => {
            const target = e.target;
            window.GENETIC_PROFILE_DEBUG.logLevel = target.value;
        });
    }
    if (clearButton) {
